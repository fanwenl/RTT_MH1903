...
#define RT_FOTA_CRC_INIT_VAL 0xffffffff
    ...
    {
        LOG_D("Partition[%s] firmware integrity verify failed.crc = %08x",part->name,part_head.code_crc);
        fota_res = RT_FOTA_FW_VERIFY_FAILED;
        goto __exit_partition_verify;
    }
...
static int rt_fota_read_part(const struct fal_partition *part, int read_pos, tiny_aes_context *aes_ctx, rt_uint8_t *aes_iv, rt_uint8_t *decrypt_buf, rt_uint32_t decrypt_len)
{
    ...
    if ((part == RT_NULL) || (decrypt_buf == RT_NULL) || (decrypt_len % 16 != 0) || (decrypt_len > RT_FOTA_ALGO_BUFF_SIZE))
    {
    ...
    while (fw_raw_pos < part_head->com_size)
    {
        if ((part_head->fota_algo & RT_FOTA_CMPRS_STAT_MASK) != RT_FOTA_CRYPT_ALGO_NONE)
        {
            if (block_hdr_pos >= RT_FOTA_ALGO_BUFF_SIZE)
            {
                ...
                LOG_D("%s:%s memcpy %d from %p to %p.",__FUNCTION__,__LINE__, block_size, &crypt_buf[RT_FOTA_BLOCK_HEADER_SIZE], cmprs_buff);
...
static int rt_fota_start_application(void)
{
    ...
    app_addr = part->offset + MH_FLASH_START_ADRESS;
    //判断固件地址是不是合法.
    if ((*(__IO uint32_t *)(app_addr + 4)) < app_addr || (*(__IO uint32_t *)(app_addr + 4)) > app_addr +  part->len)
    {
        LOG_I("Illegal Flash code:0x%x", *(__IO uint32_t *)(app_addr + 4));
        fota_res = RT_FOTA_GENERAL_ERR;
        goto __exit_start_application;
    }
    // 检查栈顶地址是否合法.
    if (((*(__IO uint32_t *)app_addr) & (MH_SRAM_START + (~(MH_SRAM_SIZE - 1) & 0x0fffffff))) != MH_SRAM_START)
    {
    ...
    __disable_irq();
    for (IRQn_Type irq = DMA_IRQn; irq <= SPI2_IRQn; irq++)
    {
        NVIC_DisableIRQ(irq);
        NVIC_ClearPendingIRQ(irq);
    }

    NVIC_DisableIRQ(SCI2_IRQn);
    NVIC_ClearPendingIRQ(SCI2_IRQn);

    for (IRQn_Type irq = UART2_IRQn; irq <= UART3_IRQn; irq++)
    {
        NVIC_DisableIRQ(irq);
        NVIC_ClearPendingIRQ(irq);
    }

    for (IRQn_Type irq = QSPI_IRQn; irq <= TIM0_7_IRQn; irq++)
    {
        NVIC_DisableIRQ(irq);
        NVIC_ClearPendingIRQ(irq);
    }
    ...
    for (IRQn_Type irq = QR_IRQn; irq <= GPU_IRQn; irq++)
    {
        NVIC_DisableIRQ(irq);
        NVIC_ClearPendingIRQ(irq);
    }

    for (IRQn_Type irq = AWD_IRQn; irq <= SPI5_IRQn; irq++)
    {
        NVIC_DisableIRQ(irq);
        NVIC_ClearPendingIRQ(irq);
    }
